...
class MyLogisticRegression:
    ...
    def fit(self, x, y, learningRate = 0.0000001, noEpochs = 700):
        self.coef_ = [0.0 for _ in range(1 + len(x[0]))]    #beta or w coefficients y = w0 + w1 * x1 + w2 * x2 + ...
        #self.coef_ = [randint(0,1) for _ in range(len(x[0]) + 1)]    #beta or w coefficients 
    ...
    def predictOneSample(self, sampleFeatures):
        ...
        #print(computedFloatValue)
        computed01Value = sigmoid(computedFloatValue)
        print(computed01Value)
        if(computed01Value < threshold):
            computedLabel=0
        elif(computed01Value< threshold+0.03):
            computedLabel=1
        else:
            computedLabel=2
