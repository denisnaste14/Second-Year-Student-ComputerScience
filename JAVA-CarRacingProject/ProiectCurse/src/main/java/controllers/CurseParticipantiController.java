...
public class CurseParticipantiController {
    ...
    public void initial(Service service,Stage s, TextField usrn, TextField pass){
    ...
    public void cautare(MouseEvent mouseEvent) throws Exception {
        ...
        Stage s=new Stage();
        CautareController cc=loader.getController();
    ...
    public void inscriere(MouseEvent mouseEvent) {
        ...
        service.participantAdd(null,nume,echipa);
        int  ultimulID=-1;
        for(Participant p : service.participantFindAllByEchipa(echipa))
            if(p.getId()>ultimulID)
                ultimulID=p.getId();

        for(Cursa c: service.cursaFindAllByCapCilindrica(capCil)){
            service.inscriereAdd(null, service.participantFindOne(ultimulID),c);
        }
