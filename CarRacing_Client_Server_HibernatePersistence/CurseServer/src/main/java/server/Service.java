...
import repository.StaffHibernateRepo;
...
public class Service implements ICurseServices {
   ...
   /* public Service(ParticipantDbRepo pdr, CursaDbRepo cdr, InscriereDbRepo idr, StaffDbRepo sdr){
    ...
    public Service(ParticipantDbRepo pdr, CursaDbRepo cdr, InscriereDbRepo idr, StaffHibernateRepo sdr){
        this.pdr=pdr;
        this.cdr=cdr;
        this.idr=idr;
        this.sdr=sdr;
    ...
    public synchronized Staff login(Staff membru, ICurseObserver observer) {
        ...
        s.setId(new Tuple<>(membru.getUsername(), membru.getPassword()));
        System.out.println("kkk"+s);
        System.out.println("lll"+s.getId());
        if(s !=null){
