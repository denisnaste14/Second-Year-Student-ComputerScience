...
public class CurseClientRpcProxy implements ICurseServices {
    ...
    public Staff login(Staff membru, ICurseObserver observer) {
        ...
        membru=staffFindOne(membru.getId().getLeft(),membru.getId().getRight());
        if(membru.getNume().equals(""))
            throw new CurseException("Nu exista un membru al staffului cu acest nume!");
